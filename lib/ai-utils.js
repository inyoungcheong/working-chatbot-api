...
async function generateResponse(conversationHistory, userMessage) {
  ...
  const SANDBOX_ENDPOINT = "https://api-ai-sandbox.princeton.edu/";
  const API_VERSION = "2025-03-01-preview";
  const MODEL_NAME = "gpt-4o"; // Princeton에서 사용 가능한 모델
  ...
  // Azure OpenAI API 형식 URL (Princeton 문서에 따라)
  const url = `${SANDBOX_ENDPOINT}openai/deployments/${MODEL_NAME}/chat/completions?api-version=${API_VERSION}`;

  const response = await fetch(url, {
    method: 'POST',
    headers: {
      'api-key': API_KEY, // Azure 스타일 헤더
